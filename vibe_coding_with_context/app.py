...
context_candidates = [
    Path(__file__).resolve().parent / "data" / "business_context.csv",
    Path(__file__).resolve().parents[1] / "data" / "business_context.csv",
    Path.cwd() / "data" / "business_context.csv",
]
...
if context_path is not None:
    ...
    df["marketing_campaign"] = df["marketing_campaign"].fillna("")
    df["is_holiday"] = pd.to_numeric(df["is_holiday"], errors="coerce").fillna(0)
    df["is_release_day"] = pd.to_numeric(df["is_release_day"], errors="coerce").fillna(0)
    df["expected_revenue_multiplier"] = pd.to_numeric(df["expected_revenue_multiplier"], errors="coerce").fillna(1.0)
else:
    df["is_holiday"] = 0
    df["holiday_name"] = ""
    df["is_release_day"] = 0
    df["marketing_campaign"] = ""
    df["expected_revenue_multiplier"] = 1.0
...
if summary.empty:
    ...
    display["is_expected"] = display["is_expected"].map({True: "Expected", False: "Unexpected"})
